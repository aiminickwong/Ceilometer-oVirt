...
#
# modified by luogangyi <luogangyi@chinamobile.com>
...
class GuestAgent(object):
    ...
    def __init__(self, socketName, channelListener, user='Unknown',
                 ips=''):
    ...
    def _handleAPIVersion(self, version):
        ...
        try:
            ...
            LOG.warning("Received invalid version value: %s", version)
        ...
        if commonVersion != self.effectiveApiVersion:
            ...
            LOG.info("Guest API version changed from %d to %d",
    ...
    def _connect(self):
        ...
        try:
            self._stopped = True
            LOG.debug("Attempting connection to %s", self._socketName)
            result = self._sock.connect_ex(self._socketName)
            if result == 0:
                LOG.debug("Connected to %s", self._socketName)
                self._messageState = MessageState.NORMAL
            ...
            LOG.debug("Connection attempt failed: %s", err)
        return ret
    ...
    def _handleMessage(self, message, args):
        LOG.debug("Guest's message %s: %s", message, args)
        ...
        if message == 'heartbeat':
            ...
            if 'apiVersion' in args:
                ...
                LOG.debug("API versioning no longer reported by guest.")
                self.effectiveApiVersion = _IMPLICIT_API_VERSION_ZERO
            ...
            LOG.debug("username: %s", repr(self.guestInfo['username']))
            ...
            LOG.debug("RHEV agent was uninstalled.")
            self.guestInfo['appsList'] = []
        elif message == 'session-startup':
            LOG.debug("Guest system is started or restarted.")
        elif message == 'fqdn':
            self.guestInfo['guestFQDN'] = args['fqdn']
        elif message == 'session-shutdown':
            LOG.debug("Guest system shuts down.")
            ...
            LOG.error('Unknown message type %s', message)
    ...
    def desktopLock(self):
        try:
            LOG.debug("desktopLock called")
            self._forward("lock-screen")
        except:
            LOG.exception("desktopLock failed")

    def desktopLogin(self, domain, user, password):
        try:
            LOG.debug("desktopLogin called")
            if domain != '':
            ...
            LOG.exception("desktopLogin failed")

    def desktopLogoff(self, force):
        try:
            LOG.debug("desktopLogoff called")
            self._forward('log-off')
        except:
            LOG.exception("desktopLogoff failed")

    def desktopShutdown(self, timeout, msg, reboot):
        try:
            LOG.debug("desktopShutdown called")
            self._forward('shutdown', {'timeout': timeout, 'message': msg,
                                       'reboot': str(reboot)})
        except:
            LOG.exception("desktopShutdown failed")

    def sendHcCmdToDesktop(self, cmd):
        try:
            LOG.debug("sendHcCmdToDesktop('%s')" % (cmd))
            self._forward(str(cmd))
        except:
            LOG.exception("sendHcCmdToDesktop failed")

    def setNumberOfCPUs(self, count):
        LOG.debug("setNumberOfCPUs('%d') called", count)
    ...
    def _onChannelTimeout(self):
        ...
        if self.guestStatus not in (vmstatus.POWERING_DOWN,
            ...
            LOG.debug("Guest connection timed out")
            self.guestStatus = None
    ...
    def _handleData(self, data):
        while (not self._stopped) and '\n' in data:
            ...
            if self._messageState is MessageState.TOO_BIG:
                self._messageState = MessageState.NORMAL
                LOG.warning("Not processing current message because it "
                                 "was too big")
            else:
        ...
        if self._bufferSize >= self.MAX_MESSAGE_SIZE:
            LOG.warning("Discarding buffer with size: %d because the "
